...
module.exports = async (socket, args) => {
    ...
    if (socket.version <= 12) {
        ...
        switch (requestedFile) {
            case 'Shields.xml':
                console.log(`${chalk.green.bold('[GCF]')} ${socket.remoteAddress} has requested Shields.xml`);
                socket.write(`GCF ${transactionID} Shields.xml ${encodedShieldsLength}\r\n${encodedShields}`);
                break;
            default:
                console.log(`${chalk.red.bold('[GCF]')} ${socket.remoteAddress} has requested an unknown file. (${requestedFile})`);
