...
module.exports = async (req, res) => {
    ...
    if (req.body["soap:Envelope"]["soap:Header"]["ABAuthHeader"]["TicketToken"]) {
        ...
        console.log(`${chalk.red.bold('[ABFindContactsPaged]')} No token provided.`);
        res.send(`OUT\r\n`).status(401);
        return;
    }
    ...
    if (!decoded) {
        console.log(`${chalk.red.bold('[ABFindContactsPaged]')} ${req.ip} has an invalid token.`);
        res.send(`OUT\r\n`).status(401);
        return;
    }
    ...
    if (!user) {
        console.log(`${chalk.red.bold('[ABFindContactsPaged]')} User not found.`);
        res.send(`OUT\r\n`);
        return;
    }

    if (req.body["soap:Envelope"]["soap:Body"]["ABFindContactsPaged"]["filterOptions"]["DeltasOnly"] && 
        req.body["soap:Envelope"]["soap:Body"]["ABFindContactsPaged"]["filterOptions"]["DeltasOnly"] === true) {
    ...
    const template = fs.readFileSync('./services/soap/abservice/templates/ABFindContactsPagedResponse.xml', 'utf8');
