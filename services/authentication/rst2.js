...
exports.rst2 = async (req, res) => {
    ...
    try {
        email = req.body["s:Envelope"]["s:Header"]["wsse:Security"]["wsse:UsernameToken"]["wsse:Username"];
        password = req.body["s:Envelope"]["s:Header"]["wsse:Security"]["wsse:UsernameToken"]["wsse:Password"];
    } catch (error) {
        console.log(`${chalk.yellow.bold('[RST2.srf]')} No email or password provided.`);
        const invalid = fs.readFileSync('./services/authentication/templates/rst2/InvalidRequest.xml', 'utf8');
        res.send(invalid);
        return;
    }
    ...
    if (!user) {
        ...
        const template = fs.readFileSync('./services/authentication/templates/rst2/FailedAuth.xml', 'utf8');
    ...
    const finalTemplate = fs.readFileSync('./services/authentication/templates/rst2/ValidAuth.xml', 'utf8');

    let requestedTokens = [];

    try {
        requestedTokens = req.body["s:Envelope"]["s:Body"]["ps:RequestMultipleSecurityTokens"]["wst:RequestSecurityToken"];
    } catch (error) {
