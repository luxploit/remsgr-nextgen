...
module.exports = async (socket, args) => {
	...
	console.log(
		`${chalk.yellow.bold('[SB: USR]')} ${socket.remoteAddress} is trying to log in with ${email}.`
	)
	...
	if (!regularSocket) {
		console.log(
			`${chalk.yellow.bold('[SB: USR]')} ${socket.remoteAddress} has an invalid switchboard token.`
		)
	...
	console.log(
		`${chalk.yellow.bold('[SB: USR]')} ${socket.remoteAddress} has successfully logged in as ${socket.passport}.`
	)
}
