...
module.exports = async (socket, args) => {
	...
	if (!socket.chat) {
		console.log(
			`${chalk.yellow.bold('[SB: CAL]')} ${socket.remoteAddress} has no chatroom assigned, cannot call.`
		)
	...
	regularSocket.write(
		`RNG ${socket.chat} ${config.server.ip}:${config.server.switchboard_port} CKI ${sb_token} ${socket.passport} ${socket.friendly_name}${regularSocket.version >= 13 ? ' U ' + config.server.host : ''}${regularSocket.version >= 14 ? ' 1' : ''}\r\n`
	)
	console.log(
		`${chalk.yellow.bold('[SB: CAL]')} ${socket.remoteAddress} has successfully called ${email}.`
	)
}
