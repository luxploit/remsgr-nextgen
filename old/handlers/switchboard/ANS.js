...
module.exports = async (socket, args) => {
	...
	if (check.sb_token !== sb_token) {
		console.log(
			`${chalk.yellow.bold('[SB: ANS]')} ${socket.remoteAddress} has an invalid switchboard token.`
		)
		socket.write(`911 ${transactionID}\r\n`)
		socket.destroy()
		return
	...
	for (let current = 1; current <= total; current++) {
		...
		if (participant.email !== email) {
			sbSocket.write(
				`JOI ${email} ${socket.friendly_name}${sbSocket.version >= 12 ? ' ' + socket.capabilities : ''}\r\n`
			)
		}
	}
	...
	console.log(
		`${chalk.yellow.bold('[SB: ANS]')} ${socket.remoteAddress} has successfully answered the call.`
	)
}
