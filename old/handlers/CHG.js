...
module.exports = async (socket, args, command) => {
	...
	if (!['NLN', 'BSY', 'IDL', 'BRB', 'AWY', 'PHN', 'LUN', 'HDN'].includes(status)) {
		console.log(
			`${chalk.red.bold('[CHG]')} ${socket.passport} has attempted to change their status to an invalid status. (${status})`
		)
		socket.destroy()
		return
	}
	...
	for (const contact of contacts) {
		...
		if (status === 'HDN') {
			contactSocket.write(
				`FLN ${user.username + '@remsgr.net'}${contactSocket.version >= 14 ? ' 1' : ''}\r\n`
			)
		} else {
			if (contactSocket.version >= 8) {
				contactSocket.write(
					`NLN ${status} ${user.username + '@remsgr.net'} ${contactSocket.version >= 14 ? '1 ' : ''}${user.friendly_name} ${capabilities}${contactSocket.version >= 9 ? ' ' + msnobjectpfp : ''}\r\n`
				)
			} else {
				contactSocket.write(
					`NLN ${status} ${user.username + '@remsgr.net'} ${user.friendly_name}\r\n`
				)
			}
		}
	}
	...
	if (socket.initial_status === false) {
		...
		for (const contact of contactsFL) {
			...
			const contactContactsAL = await Contact.find({
				userID: contact.contactID,
				contactID: socket.userID,
				list: 'AL',
			}).exec()
			...
			if (socket.version >= 8) {
				socket.write(
					`ILN ${transactionID} ${contactSocket.status} ${contactSocket.passport} ${socket.version >= 14 ? '1 ' : ''}${contactSocket.friendly_name} ${contactSocket.capabilities}${socket.version >= 9 ? ' ' + contactSocket.msnobjectpfp : ''}\r\n`
				)
				...
				socket.write(
					`ILN ${transactionID} ${contactSocket.status} ${contactSocket.passport} ${contactSocket.friendly_name}\r\n`
				)
			}
		}
