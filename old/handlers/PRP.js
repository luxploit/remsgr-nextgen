...
module.exports = async (socket, args) => {
	...
	if (type === 'MFN') {
		if (newSetting.length > 387) {
			console.log(
				`${chalk.red.bold('[REA]')} ${socket.remoteAddress} has a friendly name that is too long.`
			)
		...
		for (const contact of contacts) {
			...
			if (contactSocket.version >= 8) {
				contactSocket.write(
					`NLN ${socket.status} ${socket.passport} ${contactSocket.version >= 14 ? '1 ' : ''}${socket.friendly_name} ${socket.capabilities}${contactSocket.version >= 9 ? ' ' + socket.msnobjectpfp : ''}\r\n`
				)
			} else {
				contactSocket.write(
					`NLN ${socket.status} ${socket.passport} ${socket.friendly_name}\r\n`
				)
			}
		}
	} else if (type === 'PHH') {
		...
		await User.updateOne({ _id: decoded.id }, { $set: { 'settings.phone.PHH': newSetting } }).exec()
		...
		await User.updateOne({ _id: decoded.id }, { $set: { 'settings.phone.PHW': newSetting } }).exec()
		...
		await User.updateOne({ _id: decoded.id }, { $set: { 'settings.phone.PHM': newSetting } }).exec()
