...
module.exports = async (socket, args) => {
	...
	if (socket.version >= 10) {
		...
		results.forEach((result) =>
			result.forEach((contact) => allContacts.add(contact.contactID.toString()))
		)
		...
		socket.write(
			`SYN ${transactionID} ${timestamp} ${timestamp} ${totalContacts} ${totalGroupsInt}\r\n`
		)
		socket.write(`GTC A\r\n`)
		socket.write(`BLP AL\r\n`)
		...
		for (const contact of contacts) {
			...
			const totalListsNumber = contact.list
				.split(',')
				.map((list) => listsInt[list])
			...
			if (contactsMap.has(userId)) {
				...
				if (!contactUser) {
					console.log(
						`${chalk.magentaBright.bold('[SYN]')} ${socket.passport} has a contact that does not exist.`
					)
					continue
				}
		...
		for (const contact of reverseContacts) {
			...
			if (contactsMap.has(userId)) {
				...
				if (!contactUser) {
					console.log(
						`${chalk.magentaBright.bold('[SYN]')} ${socket.passport} has a reverse contact that does not exist.`
					)
					continue
				}
				contactsMap.set(userId, {
		...
		for (const contact of contactsData) {
			...
			socket.write(
				`LST N=${contact.email} F=${contact.friendly_name} C=${contact.uuid} ${contact.lists_number}${socket.version >= 12 ? ' 1' : ''} ${contact.groups.join(',')}\r\n`
			)
		...
		if (socket.version >= 7) {
			...
			groups.forEach((group) => {
				socket.write(
					`LSG ${transactionID} ${syncID} ${index} ${totalGroups} ${index - 1} ${group.name} 0\r\n`
				)
				index++
			})
		}
		...
		try {
			...
			for (let i = 0; i < results.length; i++) {
				...
				if (contacts.length === 0) {
					console.log(
						`${chalk.magentaBright.bold('[SYN]')} ${socket.passport} has no contacts in list ${list}.`
					)
				...
				for (const [index, contact] of contacts.entries()) {
					...
					if (!user) {
						console.log(
							`${chalk.magentaBright.bold('[SYN]')} ${socket.passport} has a contact in list ${list} that does not exist.`
						)
						continue
					}

					socket.write(
						`LST ${transactionID} ${list} ${syncID} ${index + 1} ${total} ${user.username + '@remsgr.net'} ${user.friendly_name}${socket.version > 6 ? ' 0' : ''}\r\n`
					)
				}
			}
		} catch (err) {
			console.log(
				`${chalk.magentaBright.bold('[SYN]')} ${socket.passport} failed to get contacts from the database.`
			)
