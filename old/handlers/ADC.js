...
module.exports = async (socket, args) => {
	...
	if (emailArg.startsWith('C=') && list === 'FL') {
		...
		if (!contact) {
			console.log(
				`${chalk.red.bold('[ADD]')} ${socket.passport} has attempted to add a user that does not exist. (${contactUUID})`
			)
		...
		if (!group) {
			console.log(
				`${chalk.red.bold('[ADD]')} ${socket.passport} has attempted to add a user to a group that does not exist. (${groupUUID})`
			)
			socket.write(`205 ${transactionID}\r\n`)
			return
		}

		const userIsInFL = await Contact.findOne({
			userID: decoded.id,
		...
		if (userIsInFL) {
			...
			console.log(
				`${chalk.red.bold('[ADD]')} ${socket.passport} has attempted to add a user to a group that is not in their FL list. (${groupUUID})`
			)
			socket.write(`205 ${transactionID}\r\n`)
			return
		}
		...
		console.log(
			`${chalk.green.bold('[ADD]')} ${socket.passport} has added ${email} to their FL list in group ${groupUUID}.`
		)
		return
	}
	...
	if (!user) {
		console.log(
			`${chalk.red.bold('[ADD]')} ${socket.passport} has attempted to add a user that does not exist. (${username})`
		)
		socket.write(`205 ${transactionID}\r\n`)
		return
	}
	...
	if (contacts.length >= 150 && list === 'FL') {
		console.log(
			`${chalk.red.bold('[ADD]')} ${socket.passport} has attempted to add a user to a list that is full. (${username})`
		)
	...
	if (existing) {
		console.log(
			`${chalk.red.bold('[ADD]')} ${socket.passport} has attempted to add a user that is already in their ${list} list. (${username})`
		)
	...
	if (blocked && allowed) {
		console.log(
			`${chalk.red.bold('[ADD]')} ${socket.passport} has attempted to add a user that is blocked. (${username})`
		)
		socket.write(`215 ${transactionID}\r\n`)
		return
	}
	...
	if (list === 'PL') {
		console.log(
			`${chalk.red.bold('[ADD]')} ${socket.passport} has attempted to add a user to the PL list, which is not allowed.`
		)
		socket.destroy()
		return
	}
	...
	console.log(
		`${chalk.green.bold('[ADD]')} ${socket.passport} has added ${email} to their ${list} list.`
	)
}
