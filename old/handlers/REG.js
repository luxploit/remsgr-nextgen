	...
	if (socket.version >= 10) {
		try {
			...
			if (groupWithSameName && groupWithSameName.id !== groupUUID) {
				console.log(
					`${chalk.red.bold('[REG]')} ${socket.passport} has attempted to rename a group to a name that already exists. (${newName})`
				)
				socket.write(`205 ${transactionID}\r\n`)
				return
			}
			...
			if (!group) {
				console.log(
					`${chalk.red.bold('[REG]')} ${socket.passport} has attempted to rename a group that does not exist. (${groupUUID})`
				)
				socket.write(`205 ${transactionID}\r\n`)
				return
			}
