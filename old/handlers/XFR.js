...
module.exports = (socket, args) => {
	...
	console.log(
		`${chalk.yellow.bold('[XFR]')} ${socket.remoteAddress} is trying to transfer to the switchboard.`
	)
	...
	socket.write(
		`XFR ${transactionID} SB ${config.server.ip}:${config.server.switchboard_port} CKI ${sb_token}${socket.version >= 13 ? ' U ' + config.server.host : ''}${socket.version >= 14 ? ' 1' : ''}\r\n`
	)
}
