...
exports.twnAuth = async (req, res) => {
	...
	if (header) {
		...
		if (signInMatch && pwdMatch) {
			...
			if (!urlUser) {
				console.log(
					`${chalk.yellow.bold('[USR MD5 INITIAL]')} ${passport} does not exist in the database (URL).`
				)
				res.set({
					'Content-Type': 'text/plain',
					'WWW-Authenticate': 'Passport1.4 da-status=failed',
...
exports.createAccount = async (req, res) => {
	try {
		...
		if (!/^[a-z0-9]{1,16}$/.test(username)) {
			res.status(400).send(
				'Username must be between 1 and 16 characters, be downcase, and can only contain letters and numbers.'
			)
			return
		}
