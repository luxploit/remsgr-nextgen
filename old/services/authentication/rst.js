...
exports.rst = async (req, res) => {
	...
	try {
		...
		const invalid = fs.readFileSync(
			'./services/authentication/templates/rst/InvalidRequest.xml',
			'utf8'
		)
		res.send(invalid)
		return
	...
	if (!user) {
		...
		const template = fs.readFileSync(
			'./services/authentication/templates/rst/FailedAuth.xml',
			'utf8'
		)
	...
	const finalTemplate = fs.readFileSync(
		'./services/authentication/templates/rst/ValidAuth.xml',
		'utf8'
	)
	...
	try {
		requestedTokens =
			req.body['Envelope']['Body']['ps:RequestMultipleSecurityTokens']['wst:RequestSecurityToken']
	} catch (error) {
