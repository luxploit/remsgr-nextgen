	...
	async login(socket, version, state, transactionID, passport, passhash) {
		if (state === 'I') {
			...
			if (email[1] !== 'remsgr.net' && email[1] !== 'hotmail.com') {
				console.log(
					`${chalk.yellow.bold('[USR MD5 INITIAL]')} ${passport} has an invalid email domain.`
				)
			...
			if (!user) {
				console.log(
					`${chalk.yellow.bold('[USR MD5 INITIAL]')} ${passport} does not exist in the database.`
				)
				socket.write(`911 ${transactionID}\r\n`)
				socket.destroy()
				return
			...
			if (!legacyPass) {
				console.log(
					`${chalk.yellow.bold('[USR MD5 INITIAL]')} ${passport} has no legacy password.`
				)
				socket.write(`911 ${transactionID}\r\n`)
				socket.destroy()
				return
			...
			if (!user) {
				console.log(
					`${chalk.yellow.bold('[USR MD5 SUBSEQUENT]')} ${passport} does not exist in the database.`
				)
				socket.write(`911 ${transactionID}\r\n`)
				socket.destroy()
				return
			...
			if (!legacyPass) {
				console.log(
					`${chalk.yellow.bold('[USR MD5 SUBSEQUENT]')} ${passport} has no legacy password.`
				)
				socket.write(`911 ${transactionID}\r\n`)
				socket.destroy()
				return
			...
			if (passhash !== md5Password) {
				console.log(
					`${chalk.yellow.bold('[USR MD5 SUBSEQUENT]')} ${passport} has entered an incorrect password.`
				)
				socket.write(`911 ${transactionID}\r\n`)
				socket.destroy()
				return
			...
			existingSockets.forEach((existingSocketObj) => {
				if (existingSocketObj !== socket) {
					console.log(
						`${chalk.yellow.bold('[USR MD5 SUBSEQUENT]')} ${passport} is already logged in, logging out old session.`
					)
			...
			console.log(
				`${chalk.yellow.bold('[USR MD5 SUBSEQUENT]')} ${passport} has successfully logged in.`
			)
			...
			if (version >= 7) {
				...
				const lengthWithoutPlaceholder =
					initialHeader.length + messageTemplate.length - newLineCount

				const messageLength =
					lengthWithoutPlaceholder + String(lengthWithoutPlaceholder).length + 1
...
<cli maj="7" min="0" minbld="0" maxbld="9999" deny=" " /></shield> <block></block></config></Policy>
  ...
  <Entry hr="0x82000020" action="3" />  <Entry hr="0x8007274A" action="3" /> </Feature></Policy></Policy>
	<Policy type="P2P" checksum="815D4F1FF8E39A85F1F97C4B16C45177"><ObjStr SndDly="1" /></Policy>
			...
			if (!decoded) {
				console.log(
					`${chalk.yellow.bold('[USR TWN SUBSEQUENT]')} ${passport} has an invalid token.`
				)
				socket.write(`911 ${transactionID}\r\n`)
				socket.destroy()
				return
			...
			if (email[1] !== 'remsgr.net' && email[1] !== 'hotmail.com') {
				console.log(
					`${chalk.yellow.bold('[USR TWN INITIAL]')} ${passport} has an invalid email domain.`
				)
				socket.write(`911 ${transactionID}\r\n`)
				socket.destroy()
				return
			...
			if (!user) {
				console.log(
					`${chalk.yellow.bold('[USR TWN SUBSEQUENT]')} ${passport} does not exist in the database.`
				)
				socket.write(`911 ${transactionID}\r\n`)
				socket.destroy()
				return
			...
			existingSockets.forEach((existingSocketObj) => {
				if (existingSocketObj !== socket) {
					console.log(
						`${chalk.yellow.bold('[USR TWN SUBSEQUENT]')} ${passport} is already logged in, logging out old session.`
					)
					existingSocketObj.write('OUT\r\n')
					existingSocketObj.destroy()
				}
			...
			console.log(
				`${chalk.yellow.bold('[USR TWN SUBSEQUENT]')} ${passport} has successfully logged in.`
			)
			if (version >= 10) {
...
<cli maj="7" min="0" minbld="0" maxbld="9999" deny=" " /></shield> <block></block></config></Policy>
  ...
  <Entry hr="0x82000020" action="3" />  <Entry hr="0x8007274A" action="3" /> </Feature></Policy></Policy>
	<Policy type="P2P" checksum="815D4F1FF8E39A85F1F97C4B16C45177"><ObjStr SndDly="1" /></Policy>
			...
			if (!decoded) {
				console.log(
					`${chalk.yellow.bold('[USR SSO SUBSEQUENT]')} ${passport} has an invalid token.`
				)
				socket.write(`911 ${transactionID}\r\n`)
				socket.destroy()
				return
			...
			if (email[1] !== 'remsgr.net' && email[1] !== 'hotmail.com') {
				console.log(
					`${chalk.yellow.bold('[USR SSO INITIAL]')} ${passport} has an invalid email domain.`
				)
				socket.write(`911 ${transactionID}\r\n`)
				socket.destroy()
				return
			...
			if (!user) {
				console.log(
					`${chalk.yellow.bold('[USR SSO SUBSEQUENT]')} ${passport} does not exist in the database.`
				)
				socket.write(`911 ${transactionID}\r\n`)
				socket.destroy()
				return
			...
			existingSockets.forEach((existingSocketObj) => {
				if (existingSocketObj !== socket) {
					console.log(
						`${chalk.yellow.bold('[USR SSO SUBSEQUENT]')} ${passport} is already logged in, logging out old session.`
					)
					existingSocketObj.write('OUT\r\n')
					existingSocketObj.destroy()
				}
			...
			console.log(
				`${chalk.yellow.bold('[USR SSO SUBSEQUENT]')} ${passport} has successfully logged in.`
			)
			if (version >= 10) {
				socket.write(`USR ${transactionID} OK ${passport} 1 0\r\n`)
...
async function logOut(socket) {
	...
	try {
		...
		for (const contact of contacts) {
			...
			const contactContacts = await Contact.find({
			...
			if (contactContacts.length > 0) {
				contactSocket.write(
					`FLN ${socket.passport}${contactSocket.version >= 14 ? ' 1' : ''}\r\n`
				)
			}
		}
	} catch (err) {
		console.log(
			`${chalk.red.bold('[LOGOUT UTIL]')} ${socket.passport} failed to get contacts from the database.`
		)
	} finally {
		socket.destroy()
	}
